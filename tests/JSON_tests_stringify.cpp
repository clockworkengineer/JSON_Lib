...
TEST_CASE("JSON object for stringification of a list of example JSON files.", "[JSON][Stringify]")
{
    ...
    SECTION("Stringify to  buffer and check value", "[JSON][Stringify]")
    {
        ...
        json.parse(jsonSource);
        json.stringify(jsonDestinaton);
        REQUIRE(jsonDestinaton.getBuffer() == json.stripWhiteSpaceBuffer(jsonFileBuffer));
    }
    SECTION("Stringify to file and check value", "[JSON][Stringify]")
    {
        ...
        json.parse(jsonSource);
        json.stringify(jsonDestination);
        REQUIRE(readJSONFromFile(prefixTestDataPath(kGeneratedJSONFile)) == json.stripWhiteSpaceBuffer(jsonFileBuffer));
    }
}
