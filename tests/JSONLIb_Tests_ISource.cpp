...
TEST_CASE("ISource (Buffer) interface (uses file testfile001.json).",
  ...
  SECTION("Create BufferSource and then check next "
  ...
  SECTION("Create BufferSource move past last character, "
    ...
    REQUIRE(source.position() ==
            std::filesystem::file_size(testFileName));   // eof
  ...
  SECTION(
      "Create BufferSource and check that it is ignoring whitespace correctly.",
      "[JSON][ISource][Buffer]") {
    BufferSource source{
        "{    \"Name\" \t\t\t\r\r\r\r\r\r:\r\n       \"Pete\"   \r\r\r\r}"};
  ...
  SECTION("Create BufferSource and then try to read off "
          "the end",
          "[JSON][ISource][Buffer][Exception]") {
    BufferSource source{BufferSource(buffer)};
    while (source.more()) {
...
TEST_CASE("ISource (File) interface.", "[JSON][ISource][File]") {
  ...
  SECTION("Create FileSource and then check next "
          "positions to correct next character",
          "[JSON][ISource][File]") {
  ...
  SECTION("Create FileSource move past last character, "
          "check it and the bytes moved.",
          "[JSON][ISource][File]") {
    FileSource source{testFileName};
    while (source.more()) {
  ...
  SECTION("Check that FileSource is ignoring whitespace correctly.",
          "[JSON][ISource][File]") {
    std::filesystem::remove(kGeneratedJSONFile);
    writeToFile(
        kGeneratedJSONFile,
  ...
  SECTION("Create FileSource and then try to read off "
          "the end",
          "[JSON][ISource][File][Exception]") {
    FileSource source{testFileName};
    while (source.more()) {
