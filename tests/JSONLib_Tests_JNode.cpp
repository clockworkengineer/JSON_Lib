...
TEST_CASE("Use of JNode indexing operators", "[JSON][JNode][Index]") {
  ...
  SECTION("Parse dictionary and check its components using indexing",
    ...
    REQUIRE(JNodeRef<String>((json.root())["City"]).string() ==
            "Southampton");
    REQUIRE(JNodeRef<Number>((json.root())["Population"]).toString() ==
            "500000");
  }
  ...
  SECTION("Parse list with embedded dictionary and check its components using "
    ...
    json.parse(jsonSource);
    REQUIRE(JNodeRef<Number>((json.root())[0]).toString() == "777");
    REQUIRE(JNodeRef<String>((json.root())[1]["City"]).string() ==
            "Southampton");
    REQUIRE(JNodeRef<Number>((json.root())[1]["Population"]).toString() ==
            "500000");
    REQUIRE(JNodeRef<String>((json.root())[2]).string() == "apples");
  }
...
TEST_CASE("Check JNode reference functions work.", "[JSON][JNode][Reference]") {
  ...
  SECTION("Integer reference.", "[JSON][JNode][Reference]") {
    ...
    json.parse(jsonSource);
    REQUIRE(JNodeRef<Number>(json.root()).toString() == "45500");
  }
  SECTION("String reference.", "[JSON][JNode][Reference]") {
    BufferSource jsonSource{"\"0123456789\""};
    json.parse(jsonSource);
    REQUIRE(JNodeRef<String>(json.root()).string() == "0123456789");
  }
