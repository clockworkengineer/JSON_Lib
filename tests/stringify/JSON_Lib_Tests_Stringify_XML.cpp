...
TEST_CASE("Check JSON stringification to XML of simple types.", "[JSON][Stringify][Simple][XML]")
{
  ...
  SECTION(
    "XML encode an string with unprintable characters (128-255) and check its value ", "[JSON][Stringify][XML][String]")
  {
    std::string escaped{ "\"abcdefghijklmnopqrstuvwxyz" };
    // Add all ASCII except '"' and '\'
  ...
  SECTION(R"(Stringify [{},{},{}] to a XML and check its value.)", "[JSON][Stringify][Array][XML]")
  {
    BufferDestination jsonDestination;
    json.parse(BufferSource{ R"([{},{},{}])" });
    json.stringify(jsonDestination);
