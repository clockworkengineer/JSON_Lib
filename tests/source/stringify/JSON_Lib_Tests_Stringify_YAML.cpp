...
TEST_CASE("Check JSON stringification to YAML of simple types.", "[JSON][Stringify][Simple][YAML]")
{
  ...
  // SECTION("Stringify a string (abcdefghijklmnopqrstuvwxyz &<>'\") to YAML with default character escapes.",
  //   "[JSON][Stringify][String][YAML]")
  // {
  //   BufferDestination jsonDestination;
  //   json.parse(BufferSource{ R"("abcdefghijklmnopqrstuvwxyz &<>'\"")" });
  //   json.stringify(jsonDestination);
  ...
  //     if (static_cast<char>(ch) != '"' && static_cast<char>(ch) != '\\' && static_cast<char>(ch) != '\"'){ escaped += static_cast<char>(ch); }
  //   }
  //   escaped += '"';
  //   BufferSource source{ escaped };
  //   BufferDestination destination;
  ...
  //           == R"(<?xml version="1.0" encoding="UTF-8"?><root>abcdefghijklmnopqrstuvwxyz &#x007F;</root>)");
  // }
  // SECTION(R"(Stringify YAML string with escapes (1-127)' to buffer and check value.)", "[JSON][Stringify][YAML][Escapes]")
  // {
