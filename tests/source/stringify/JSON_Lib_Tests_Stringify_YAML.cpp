...
TEST_CASE("Check JSON stringification to YAML of simple types.", "[JSON][Stringify][Simple][YAML]")
{
  ...
  //     if (static_cast<char>(ch) != '"' && static_cast<char>(ch) != '\\' && static_cast<char>(ch) != '\"'){ escaped +=
  //     static_cast<char>(ch); }
  //   }
  //   escaped += '"';
  //   BufferSource source{ escaped };
  ...
  //     == R"(<?xml version="1.0" encoding="UTF-8"?><root><Row>True</Row><Row>Out of
  //     time</Row><Row>7890430242211233792</Row><Row><key>4444</key></Row></root>)");
  // }
  ...
  // }
  // SECTION(R"(Stringify YAML string with escapes (1-127)' to buffer and check value.)",
  // "[JSON][Stringify][YAML][Escapes]")
  // {
