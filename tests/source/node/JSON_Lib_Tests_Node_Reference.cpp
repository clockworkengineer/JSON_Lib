...
TEST_CASE("Check Node reference function exceptions.", "[JSON][Node][Reference][Exception]")
{
  ...
  SECTION("String reference an number.", "[JSON][Node][Reference][String]")
  {
    BufferSource jsonSource{ "45500" };
    json.parse(jsonSource);
    REQUIRE_THROWS_AS(JRef<String>(json.root()).value(), Node::Error);
    REQUIRE_THROWS_WITH(JRef<String>(json.root()).value(), "Node Error: Node not a string.");
  }
  SECTION("Number reference a string.", "[JSON][Node][Reference][Number][Exception]")
  {
    BufferSource jsonSource{ R"("0123456789")" };
    json.parse(jsonSource);
    REQUIRE_THROWS_AS(JRef<Number>(json.root()).is<int>(), Node::Error);
    REQUIRE_THROWS_WITH(JRef<Number>(json.root()).is<int>(), "Node Error: Node not a number.");
  }
  SECTION("Null reference a boolean.", "[JSON][Node][Reference][Null][Exception]")
  {
    BufferSource jsonSource{ R"(true)" };
    json.parse(jsonSource);
    REQUIRE_THROWS_AS(JRef<Null>(json.root()).value(), Node::Error);
    REQUIRE_THROWS_WITH(JRef<Null>(json.root()).value(), "Node Error: Node not a null.");
  }
  SECTION("Boolean reference a null.", "[JSON][Node][Reference][Boolean][Exception]")
  {
    BufferSource jsonSource{ R"(null)" };
    json.parse(jsonSource);
    REQUIRE_THROWS_AS(JRef<Boolean>(json.root()).value(), Node::Error);
    REQUIRE_THROWS_WITH(JRef<Boolean>(json.root()).value(), "Node Error: Node not a boolean.");
  }
  SECTION("Object reference an array.", "[JSON][Node][Reference][Object][Exception]")
  {
    BufferSource jsonSource{ R"([777,9000,"apples"])" };
    json.parse(jsonSource);
    REQUIRE_THROWS_AS(JRef<Object>(json.root()).size(), Node::Error);
    REQUIRE_THROWS_WITH(JRef<Object>(json.root()).size(), "Node Error: Node not an object.");
  }
  SECTION("Array reference an object.", "[JSON][Node][Reference][Array][Exception]")
  {
    BufferSource jsonSource{ R"({"City":"Southampton","Population":500000 })" };
    json.parse(jsonSource);
    REQUIRE_THROWS_AS(JRef<Array>(json.root()).size(), Node::Error);
    REQUIRE_THROWS_WITH(JRef<Array>(json.root()).size(), "Node Error: Node not an array.");
  }
}