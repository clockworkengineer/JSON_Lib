//
// Unit Tests: JSON_Lib_Tests_Parse_Exceptions
//
...
TEST_CASE("Check parser generated exceptions.", "[JSON][Parse][Exception]")
{
  ...
  SECTION("Parse missing terminating '\"' in string.", "[JSON][Parse][Exception]")
  {
    ...
    REQUIRE_THROWS_AS(json.parse(jsonSource), JSON::Error);
    jsonSource.reset();
  ...
  SECTION("Parse object with invalid value field (number).", "[JSON][Parse][Exception]")
  {
    ...
    REQUIRE_THROWS_AS(json.parse(jsonSource), JSON::Error);
    jsonSource.reset();
    REQUIRE_THROWS_WITH(json.parse(jsonSource), "JSON Error [Line: 1 Column: 18]: Invalid numeric value.");
  }
