...
TEST_CASE("Check JNode Number conversion.", "[JSON][JNode][Number]")
{
  ...
  SECTION("Floating point converted to long.", "[JSON][JNode][Number]")
  {
    BufferSource jsonSource{ "6788888.8990" };
    json.parse(jsonSource);
    REQUIRE_FALSE(!JRef<Number>(json.root()).getNumber().isFloat());
  ...
  SECTION("Floating point converted to float.", "[JSON][JNode][Number]")
  {
  ...
  SECTION("Floating point converted to double.", "[JSON][JNode][Number]")
  {
    BufferSource jsonSource{ "678.8990" };
    json.parse(jsonSource);
    REQUIRE_FALSE(!JRef<Number>(json.root()).getNumber().isFloat());
  ...
  SECTION("Integer converted to int.", "[JSON][JNode][Number]")
  {
  ...
  SECTION("Integer converted to long.", "[JSON][JNode][Number]")
  {
    BufferSource jsonSource{ "78989" };
    json.parse(jsonSource);
    REQUIRE_FALSE(!JRef<Number>(json.root()).getNumber().isInt());
  ...
  SECTION("Integer converted to float.", "[JSON][JNode][Number]")
  {
    BufferSource jsonSource{ "78989" };
    json.parse(jsonSource);
    REQUIRE_FALSE(!JRef<Number>(json.root()).getNumber().isInt());
  ...
  SECTION("Integer converted to double.", "[JSON][JNode][Number]")
  {
    BufferSource jsonSource{ "78989" };
    json.parse(jsonSource);
    REQUIRE_FALSE(!JRef<Number>(json.root()).getNumber().isInt());
  ...
  SECTION("Check floating point with invalid exponent.", "[JSON][JNode][Number][Exception]")
  {
  ...
  SECTION("Check floating point with multiple decimal points.", "[JSON][JNode][Number][Exception]")
  {
